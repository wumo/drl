...
def ppo_cart_pole():
    ...
    PPO2Agent(config).run_steps(tag=ppo_cart_pole.__name__)
...
def ppo_pixel_atari(game, tag=""):
    ...
    PPO2Agent(config).run_steps(tag=f'{tag}{ppo_pixel_atari.__name__}-{game}')
...
def ppo_continuous(game, tag=""):
    ...
    PPO2Agent(config).run_steps(tag=f'{tag}{ppo_continuous.__name__}-{game}')
