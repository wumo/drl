...
class WarpFrame(gym.ObservationWrapper):
    def __init__(self, env, width=84, height=84, grayscale=True):
        ...
        if self.grayscale:
            self.observation_space = spaces.Box(low=0, high=255, shape=(1, self.width, self.height,), dtype=np.uint8)
        else:
...
def wrap_deepmind(env, episode_life=True, frame_stack=True, history_length=4):
