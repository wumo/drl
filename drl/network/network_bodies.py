...
class DQNBody(nn.Module):
    def __init__(self, height, width, in_channels=4):
        super().__init__()
        ...
        convw = conv2d_size_out(conv2d_size_out(conv2d_size_out(height, 8, 4), 4, 2), 3, 1)
        convh = conv2d_size_out(conv2d_size_out(conv2d_size_out(width, 8, 4), 4, 2), 3, 1)
        self.fc4 = layer_init(nn.Linear(convh * convw * 64, self.feature_dim))
